...
def task_slug(value: str) -> str:
    ...
    return slug[:48].rstrip("-") or "scratch"


def create_project(task_name: str) -> Path:
    ...
    mutagen_project = (
        mutagen_root / "Mutagen.Bethesda.Skyrim" / "Mutagen.Bethesda.Skyrim.csproj"
    )
    if not mutagen_project.is_file():
        raise RuntimeError("Mutagen is not set up; run 'mise run setup' in the skill directory")

    slug = task_slug(task_name)
    destination = Path(tempfile.mkdtemp(prefix=f"skyrim-mutagen-{slug}-")).resolve()
...
def main() -> int:
    args = parse_args()
    try:
        destination = create_project(args.task_name)
